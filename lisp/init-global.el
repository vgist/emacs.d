...
(when window-system
  (progn
    ...
    (set-frame-position (selected-frame)
                        (/ (x-display-pixel-width) 10)
                        (/ (x-display-pixel-height) 25))
    ;;(set-frame-size (selected-frame) 400 40)    ;; width 400px,height 40px
    ...
    ;; width 80%, height 90%
    (add-to-list 'default-frame-alist
                 (cons 'width (/ (* 8 (x-display-pixel-width))
                                 (* 10 (frame-char-width)))))
    (add-to-list 'default-frame-alist
...
;; 备份文件、临时文件 ~/.cache/emacs$UID
(defconst emacs-tmp-dir (expand-file-name
                         (format "~/.cache/emacs%d" (user-uid))
...
(global-set-key (kbd "s-0") 'text-scale-set)            ; 恢复文字默认大小
(global-set-key (kbd "s--") 'text-scale-decrease)       ; 缩小文字
(global-set-key (kbd "s-=") 'text-scale-increase)       ; 放大文字
(global-set-key (kbd "C-d") 'delete-window)             ; 关闭当前激活的窗口
(global-set-key (kbd "C-S-d") 'delete-other-windows)    ; 关闭其他所有窗口
